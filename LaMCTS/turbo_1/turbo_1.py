...
class Turbo1:
    ...
    def _create_candidates(self, X, fX, length, n_training_steps, hypers):
        print('inside create_candidates..')
        ...
        # mask[:, int(X[-1][0])] = [False for i in range(700)]

        # mask = np.array([[False] * self.dim] *100*self.dim)
        ...
        #
        #
        # print('asfsafddsdsf', np.rint(X[-1][0]).astype(int))
        #
        # masked_features = self.dependencies[np.rint(X[-1][0]).astype(int)] + (0,)
        #
        #
        # print('masked_features :', masked_features)
        #
        # mask[:, masked_features] = True
        #
        # print('the mask:', mask.shape, mask)
        # print(X[0])
        # print(X[-1])
        # print(np.all(mask))
        ...
        # print('final', X_cand, y_cand, hypers)
        print('x cand and y cand', X_cand.shape, y_cand.shape)
    ...
    def optimize(self):
        ...
        while self.n_evals < self.max_evals:
            ...
            while self.n_evals < self.max_evals and self.length >= self.length_min:
                ...
                print('the x, fx:', X.shape, fX.shape)
                ...
                print('fx next:', fX_next.shape, fX_next)
