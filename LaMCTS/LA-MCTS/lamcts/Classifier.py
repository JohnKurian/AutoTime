...
class Classifier():
    ...
    ###########################
    # sampling with turbo_1
    ###########################
    # version 1: select a partition, perform one-time turbo_1 search

    def propose_samples_turbo(self, num_samples, path, func, categories, dependencies):
        #throw a uniform sampling in the selected partition
        X_init = self.propose_rand_samples_sobol(30, path, func.lb, func.ub)

        print(30, path, func.lb, func.ub, path, categories, dependencies)
        ...
        # print('turbo class:', proposed_X, fX)
        fX = fX*-1
