...
class MCTS:
    ...
    def __init__(self, lb, ub, dims, ninits, func, Cp = 1, leaf_size = 20, kernel_type = "rbf", gamma_type = "auto", name=''):
        self.dims                    =  dims
        self.samples                 =  []
        self.nodes                   =  []
        self.Cp                      =  Cp
        ...
        self.solver_type             = 'bo' #solver can be 'bo' or 'turbo_1'
    ...
    def load_agent(self):
        ...
        if os.path.isfile('mcts-agent.dat') == True:
            with open('mcts-agent.dat', 'rb') as json_data:
                self = dill.load(json_data)
                # self = pickle.load(json_data)
                print("=====>loads:", len(self.samples)," samples" )
                return self
    ...
    def dump_agent(self):
        ...
        agent_name = self.name + '.dat'
        with open(agent_name, 'wb') as fh:
            dill.dump(self, fh)
        ...
        for idx in range(self.sample_counter, iterations):
            ...
            for i in range(0, 1):
                if self.solver_type == 'bo':
                    samples = leaf.propose_samples_bo(100, path, self.lb, self.ub, self.samples)
                elif self.solver_type == 'turbo_1':
                    print('entering propose samples turbo')

                    samples, values = leaf.propose_samples_turbo(100, path, self.func, [], [])


                    # if (len(self.categories) > 0) and (len(self.dependencies) > 0):
                    #     samples, values = leaf.propose_samples_turbo(10000, path, self.func, self.categories, self.dependencies)
                    # else:
                ...
                for idx in range(0, len(samples)):
                    if self.solver_type == 'bo':
                        ...
                        print('entering propose samples turbo 2')
