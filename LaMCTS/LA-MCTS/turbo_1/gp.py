...
from gpytorch.kernels import MaternKernel, ScaleKernel, ArcKernel, RBFKernel, RBFKernelGrad, CylindricalKernel, CosineKernel, LinearKernel
...
class ArcGP(ExactGP):
    def __init__(self, train_x, train_y, likelihood, lengthscale_constraint, outputscale_constraint, ard_dims):
        super(ArcGP, self).__init__(train_x, train_y, likelihood)
        self.ard_dims = ard_dims
        self.mean_module = ConstantMean()
    ...
    def embedding(self, x):
        ...
        x_s = self.radius * torch.sin( * self.angle * x_) * mask
